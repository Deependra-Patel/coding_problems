...
long long findLongestPalindromicString(string text)
{
    int N = text.length();
    if(N == 0)
        return 0;
...
int main()
{
	...
	while(ntest--){
		...
		cin>>str;
		long long num;
		num = findLongestPalindromicString(str);

		//cout<<num<<endl;
		long long  n = str.length();
		long long  denum = (n*(n+1))/2;
		long long  gc = gcd(num, denum);
		cout<<num/gc<<"/"<<denum/gc<<endl;
	} 
    return 0;
}
